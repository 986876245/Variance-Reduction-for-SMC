function[theta, xHatFiltered,BeforeResampling,AfterResampling] = particleMetropolisHastingsSVmodel(observations, initialParameters, noParticles, noIterations, stepSize)
  ...
  [xHatFiltered(1, :), logLikelihood(1),BeforeResampling,AfterResampling] = particleFilterSVmodel(observations, theta(1, :), noParticles);
  ...
  for k = 2:noIterations  
    ...
    if (abs(thetaProposed(k, 2)) < 1.0) && (thetaProposed(k, 3) > 0.0)
      [xHatFilteredProposed(k, :), logLikelihoodProposed(k),BeforeResampling,AfterResampling] = particleFilterSVmodel(observations, thetaProposed(k, :), noParticles);
    end
